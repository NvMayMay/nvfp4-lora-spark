...
> **Reproducible result (public base + dataset, one GB10).** On `nvidia/Llama-3.1-8B-Instruct-NVFP4` + the Spider text-to-SQL task, NVFP4 LoRA - trained *and* served on a single GB10 - improves held-out gold-SQL NLL from **0.889 -> 0.850** and Spider exact-set-match from **36.8% -> 52.0%** (+15.3 pp; 2 epochs, full 1034-row dev set, deterministic, no DB execution). The same one-command recipe generalizes across families (e.g. Mistral-Small-3.2-24B-NVFP4: exact-set-match **24.5% -> 61.0%**); on a strong base that already near-saturates the strict set-match, the gain instead shows up as a large NLL/calibration improvement. Reproduce it end to end with [REPRODUCE_SPIDER.md](REPRODUCE_SPIDER.md) - `bash scripts/repro_spider.sh` reproduces the headline by default (set `DYNAMIC=1` to hot-load the adapter at runtime via the API instead of attaching it at launch). The exact eval JSON for every number quoted here is committed under [results/spider/](results/spider/) (Llama 1034-row/2-epoch; Mistral 200-row/2-epoch; Qwen3-32B 1034-row/1-epoch). The absolute exact-set-match number is scorer-dependent; the *delta* is the signal.
...
* **Target coverage is fail-fast.** Every module matching a target suffix is classified individually before load. A suffix that matches nothing in the checkpoint is a hard error (typo / wrong family). A suffix that is NVFP4 in some layers and plain BF16 in others is **not** an error: the quantized instances train via `NVFP4LoRALinear` and the in-scope BF16 instances co-train natively via `BF16LoRALinear` (FP8 targets likewise train natively via `FP8LoRALinear`); out-of-scope BF16 (e.g. an MTP head or vision tower) stays frozen. The exact coverage report is written to `<output_dir>/target_coverage.json` so every adapter records what was and was not trained. (`--allow-partial-targets` is accepted but is now a no-op, retained for backward compatibility.)
...
tests/                       # CPU-only suite (141 tests) run by CI; no GPU required by construction
serve/
...
- **Broader bundled eval harness.** v1.4 ships the deterministic Spider before/after harness ([`scripts/eval_retention.py`](scripts/eval_retention.py): gold-SQL NLL + exact-set-match, no DB execution) plus `scripts/repro_spider.sh`. Additional task harnesses (GSM8K, HumanEval, BigCodeBench generators are present under `scripts/`) are not yet wired into a one-command before/after; the training scripts produce a standard PEFT-format adapter that any benchmark consumes.
