...
investigation; see [diagnostics/README.md](diagnostics/README.md) for the
backend investigation and failure modes.
...
| **Super-120B-NVFP4** | ✅ **VLLM_CUTLASS** (recommended) | **~11-14 tok/s** single-stream, up to **~40 tok/s** aggregate at conc=3 | ✅ merge-then-serve via [`scripts/merge_lora_into_nvfp4.py`](../scripts/merge_lora_into_nvfp4.py) + [`run_super_ft_merged.sh`](run_super_ft_merged.sh) |
...
- **Super base inference**: use **VLLM_CUTLASS** (recipe in
  ...
  Measured ~11-14 tok/s on Spark. `CutlassExpertsFp4._supports_current_device()`
  ...
  exists for reference (~0.7 tok/s, 18x slower).
...
- **Super + runtime LoRA serving via vLLM**: BLOCKED in vLLM 0.21 on sm_121.
  ...
  - Workarounds (in order of recommendation):
    1. **Merge LoRA into NVFP4 base + requantize** via NVIDIA Model Optimizer,
       then serve the merged model via the CUTLASS recipe at 11-14 tok/s with
       FT behavior baked in. Loses dynamic adapter swap; gains 18x throughput
...
The validated Super-FT path is merge-then-serve: first merge the trained
adapter into the NVFP4 checkpoint with
[`scripts/merge_lora_into_nvfp4.py`](../scripts/merge_lora_into_nvfp4.py), then
serve the merged output with [`run_super_ft_merged.sh`](run_super_ft_merged.sh).
The CUTLASS kernel does not support runtime LoRA on sm_121; merging bakes the
FT behavior into the base weights and keeps vLLM on the fast kernel path. See
the root [README.md](../README.md#supported-models) supported-models table and
the Phase 2 notes in [docs/PHASE2.md](../docs/PHASE2.md).
...
- [`run_super_base_inference_cutlass.sh`](run_super_base_inference_cutlass.sh) -
  **recommended** Super base inference recipe via VLLM_CUTLASS native FP4. ~11-14 tok/s.
- [`run_super_ft_merged.sh`](run_super_ft_merged.sh) - recommended Super-FT
  recipe for a LoRA-merged NVFP4 checkpoint via VLLM_CUTLASS.
