#!/usr/bin/env python3
"""Day 5b: comparable Nano-30B LoRA training on ICH v3.1.

Mirror of `train_super_nvfp4.py` so the 4-way Day 6 eval (Nano-base vs Nano-FT vs Super-base
vs Super-FT) is apples-to-apples on training config - same hyperparams, same data, same epoch
count, only the model dir differs.

Run after Day 5 (Super training) finishes - the GPU is single-tenant.
...
Optimizer state saving is enabled by default. Use `--no-save-optimizer-state`
if disk space is tighter than resume fidelity.
"""
import sys, os, json, time, argparse, random
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Reduce NVRM memdesc churn during the NVFP4 load on GB10 unified memory.
# Larger contiguous VA ranges = fewer descriptor pool entries. Must be set
...
ADAPTER_DIR = "/path/to/adapters/nemotron_3_nano_nvfp4_lora_ichv31_1epoch"
